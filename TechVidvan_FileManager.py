...
def list_files():
    ...
    message = "Files in {} folder are: ".format(folderList)
    title = "File Directory"
    textString = ""
    while i < len(sortlist):
        textString += sortlist[i] + "\n"
        i += 1
...
Label(root, text="TechVidvan File Manager", font=("Helvetica", 16), fg="blue").grid(
    row=5, column=2
)
...
Button(root, text="List all Files in Directory", command=list_files).grid(
    row=85, column=2
)
